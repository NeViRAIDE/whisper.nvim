...
function StatusModule._create_win()
  if
    not StatusModule.win_nr or not api.nvim_win_is_valid(StatusModule.win_nr)
  then
    if
      not StatusModule.buf_nr or not api.nvim_buf_is_valid(StatusModule.buf_nr)
    then
  ...
  local function push_line(title, content)
    ...
    for i, line in ipairs(message_lines) do
      ...
      if i == 1 then
        ...
        if content.icon then
          formatted =
            string.format('%s%s %s %s', fmt_msg, right_pad, title, content.icon)
        else
      ...
      if i == 1 then
        table.insert(
          hl_infos,
          { name = title, dim = content.dim, icon = content.icon }
        )
      else
...
function StatusModule._ensure_valid(msg)
  if msg.icon and displayw(msg.icon) == 0 then msg.icon = nil end

  if msg.title and displayw(msg.title) == 0 then msg.title = nil end
...
function StatusModule.push(component, content, title)
  ...
  if type(content) == 'string' then content = { mandat = content } end
